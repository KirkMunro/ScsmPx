...
function Add-ClassHierarchyToTypeNameList {
    [CmdletBinding()]
    ...
    process {
        try {
            foreach ($class in $InputObject.EnterpriseManagementObject.GetClasses([Microsoft.EnterpriseManagement.Configuration.BaseClassTraversalDepth]::Recursive)) {
                if ($InputObject.PSTypeNames -contains $class.Name)
                {
                    continue
                }
                $insertIndex = $InputObject.PSTypeNames.IndexOf('Microsoft.EnterpriseManagement.Core.Cmdlets.Instances.EnterpriseManagementInstance')
                $InputObject.PSTypeNames.Insert($insertIndex, $class.Name)
            }
            $InputObject
        } catch {
...
# CQQxFgQU2WsoP9ryRuSCe9SfCDpkyTj0Wx8wDQYJKoZIhvcNAQEBBQAEggEAacMJ
# BRwii1IvD/pg/d1Gkm1PqOfHVgXpF+9O7ca9l9fpg7zOkqHi3YqO4t2fzV5k71wC
# 2aHAQcG+/SKINsf0tOn6BFL2XkPIUUermpwXiD9yg9tdwuzmwZs/t2dRuauTYXQz
# 7MCkvcOqd6Kt+KCDDw0LkLcNjZPwCpmF//2bjFAFIxrUKpjATRQbUY2v3KsscNul
# bf5sFOaOZ3biRoHRN6BiFd6L3XOgI9QE866IPMBQamciud/PYU/AqjGK6cktGLx5
# OAlKNYiGPHZdyNHnNwcJdgQ2XKck3/HbPupog6qOpS2y0MRzb86cXtKXJAIsGGee
# dX71FhCtJrNFbo9dLQ==
